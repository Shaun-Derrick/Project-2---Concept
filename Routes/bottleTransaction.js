...
const transactionSchema = require('../model/User') //IMPORTS THE SCHEMA
...
router.post('/login', async (req, res) => {
  const transaction = new transactionSchema({
    ...
    bottleList: bottleTransaction.bottle,
  })

  try {
    const savedTransaction = await transaction.save()
    res.status(200).send({ total: transaction.total })
  } catch (err) {
